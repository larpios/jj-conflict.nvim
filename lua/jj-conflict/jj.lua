...
function M.squash()
	...
	jj_run({ "log", "--no-graph", "-T", 'change_id.short() ++ " (" ++ commit_id.short() ++ ") | " ++ description.first_line()\n', "-n", "20" }, function(code, stdout, stderr)
		if code ~= 0 then
		...
		ui.pick({
			...
			on_select = function(item)
				if not item.change_id then return end
				-- Run jj squash
...
function M.log()
	jj_run({ "log", "--no-graph", "-T", 'change_id.short() ++ " (" ++ commit_id.short() ++ ") | " ++ author.email() ++ " | " ++ description.first_line()\n', "-n", "50" }, function(code, stdout, stderr)
		if code ~= 0 then
			util.notify("Failed to run jj log: " .. stderr, vim.log.levels.ERROR)
			return
		end
